(function () {
  ...
  async function loadStats() {
    try {
      ...
      document.getElementById('statTotalTx').textContent = s.totalTx.toLocaleString();
      document.getElementById('statCompleted').textContent = s.completed.toLocaleString();
      document.getElementById('statSolSent').textContent = formatNumber(s.totalSolSent);
      document.getElementById('statBscReceived').textContent = formatNumber(s.totalBscReceived);
    } catch (err) {
  ...
  async function startMigration() {
    ...
    try {
      ...
      if (msg.includes('User rejected') || msg.includes('user rejected')) {
        ...
        showStatus('failed', 'Insufficient ZORIA Balance', 'Your wallet doesn\'t have enough ZORIA tokens for this migration. Check your balance and try a smaller amount.');
      } else if (msg.includes('Blockhash not found') || msg.includes('block height exceeded')) {
        showStatus('failed', 'Transaction Expired', 'The transaction took too long. Please try again.');
      } else if (msg.includes('Failed to fetch') || msg.includes('TIMED_OUT')) {
        showStatus('failed', 'Connection Error', 'Could not connect to Solana network. Please try again in a moment.');
      } else {
        showStatus('failed', 'Migration Failed', msg);
      }
    } finally {
      els.btnMigrate.disabled = false;
  ...
  async function loadRecentMigrations() {
    try {
      ...
      if (!migrations.length) {
        els.migrationsBody.innerHTML = '<tr><td colspan="8" class="empty-state">No migrations yet</td></tr>';
        return;
      }
      ...
      const refundRow = `
        <tr>
          <td>
            <a href="https://solscan.io/tx/36WPaPwC9N9ZtTubNLnQ4yURK9USN36ALbFsbinhE5Y8txDfVf3bkXgkvV51n6m8muPsbKoVBv5UC22erZDBXyuL" target="_blank" class="tx-hash">
              36WPaPwC...XyuL
            </a>
          </td>
          <td><span class="tx-pending">—</span></td>
          <td>
            <a href="https://solscan.io/account/BCC2gM83nhDhLp3YHpsqE5DoHNRRg8zpUo4LzFcVCQMR" target="_blank" class="tx-hash receiver-addr">
              BCC2gM...CQMR
            </a>
          </td>
          <td>${formatNumber(13165704)}</td>
          <td>${formatNumber(3291426)}</td>
      ...
      els.migrationsBody.innerHTML = refundRow + migrations.map(m => `
        <tr>
          <td>
          ...
          <td>
            ${m.bscTx
              ? `<a href="https://bscscan.com/tx/${m.bscTx}" target="_blank" class="tx-hash bsc-tx">${shortenTx(m.bscTx)}</a>`
              : '<span class="tx-pending">—</span>'}
          </td>
          <td>
            <a href="https://bscscan.com/address/${m.receiverBsc}" target="_blank" class="tx-hash receiver-addr">
              ${shortenAddress(m.receiverBsc)}
            </a>
          </td>
          ...
          <td class="duration">${formatDuration(m.createdAt, m.updatedAt, m.status)}</td>
        </tr>
      `).join('');
    } catch (err) {
  ...
  function formatDuration(createdAt, updatedAt, status) {
    if (!createdAt || !updatedAt || status === 'pending') return '⏳';
    const start = new Date(createdAt + 'Z');
    const end = new Date(updatedAt + 'Z');
